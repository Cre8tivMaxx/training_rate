...
class TrainingFeedback(Document):
	...
	def send_feedback_notification(self):
		trainer = frappe.get_value("Training Session", self.session, "trainer")

		if not trainer:
			return

		trainer_email = frappe.get_value("Contact", trainer, "email_id")

		if not trainer_email:
			return

		subject = "New Training Feedback Received"
